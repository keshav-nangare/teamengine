...
public class TestServlet extends HttpServlet {
  ...
  public void init() throws ServletException {
    try {
      ...
      String rootDir = null;
      if (null != System.getProperty("CATALINA_BASE")) {
        rootDir = System.getProperty("CATALINA_BASE");
      } else if (null != System.getenv("CATALINA_BASE")) {
        rootDir = System.getenv("CATALINA_BASE");
      }
      rootDir = rootDir + "/webapps" + pathContext + "/META-INF/context.xml";
      DocumentBuilderFactory documentBuilderFactory = DocumentBuilderFactory.newInstance();
      DocumentBuilder builder = documentBuilderFactory.newDocumentBuilder();
      Document tournaments = builder.parse(new File(rootDir));
      NodeList ndlContext = tournaments.getElementsByTagName("Context");
      ...
      if (null != System.getProperty("USERS")) {
        ...
        userPath = System.getenv("TE_BASE") + "/users";
      }
      aliasesPath = aliasesPath.split(",/reports")[0];
      aliasesPath = aliasesPath + ",/reports=" + userPath;
      ((Element) ndlContext.item(0)).setAttribute("aliases", aliasesPath);
      Transformer transformer = TransformerFactory.newInstance().newTransformer();
      transformer.setOutputProperty(OutputKeys.INDENT, "yes");
      DOMSource sourceInput = new DOMSource(tournaments);
      StreamResult console = new StreamResult(new File(rootDir));
      transformer.transform(sourceInput, console);
